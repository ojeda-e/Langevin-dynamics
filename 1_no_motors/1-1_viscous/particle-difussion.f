...
c-------variables difusion      
	integer iseedg
	real*8  x,rnd,gasdev,KT,et0,gammln,xsamp(nmaxl)
	integer*4  it

c-------variables particulas virtuales
	real*8 alpha,b0,g0,nu0,c

c-------variable motoresbi
	real*8 Fm
...
c	write(*,*) dtsamp
	...
	do m=1,nc
	   ...
	   do lr=1,nrea
             if (l3.eq.1) then
              write(5,*) sngl(k*dtsamp),lr,corrtr(lr,m,1)
             end if
             avcorr(m,l3)=avcorr(m,l3)+corrtr(lr,m,l3)
           end do !lr
           avcorr(m,l3)=avcorr(m,l3)/nrea
          end do !l3
          write(4,*) sngl(k*dtsamp),avcorr(m,1),avcorr(m,2),avcorr(m,3)
	end do !m
...
c------------------- Funcion numero random distribucion gaussiana gasdev
